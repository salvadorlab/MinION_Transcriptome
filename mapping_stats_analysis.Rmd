...
```{r}
samples <- c("Copenhageni_Basecalled_Aug_16_2019_Direct-cDNA_NoPolyATail","Copenhageni_Basecalled_Aug_16_2019_Direct-cDNA_PolyATail", "Copenhageni_Basecalled_Aug_16_2019_Direct-cDNA_Qiagen_NoPolyATail","Q29_Copenhageni_Basecalled_May_22_2020_Direct-RNA","Q36_Copenhageni_Basecalled_June_9_2020-Repeat_Direct-RNA")

samp_ab <- c("Cop.NoPolyA","Cop.PolyA", "Cop.Q.NoPolyA","Q29.Cop.DRNA","Q36.Cop.DRNA")
...
mapped.reads.trans <- data.frame(sample=c(), mapped.reads=c())
# samtools flagstats 
...
map.stats.longer$sample <- factor(map.stats.longer$sample, levels=c("Cop.NoPolyA", "Cop.Q.NoPolyA","Cop.PolyA","Q29.Cop.DRNA","Q36.Cop.DRNA"))
...
map.flag.combined <- merge(merge(merge(mapped.second.genome,mapped.second.trans), merge(mapped.supp.genome,mapped.supp.trans)), map.stats.combined) %>% select(!total.reads)
...
map.flag.longer$Reference <- rep(c("genome", "transcriptome"),10)
map.flag.longer$align_type <- rep(c(rep("secondary",2),rep("supplementary",2)),5)
map.flag.longer
...
map.flag.longer$sample <- factor(map.flag.longer$sample, levels=c("Cop.NoPolyA", "Cop.Q.NoPolyA","Cop.PolyA","Q29.Cop.DRNA","Q36.Cop.DRNA"))
...
ggplot(map.flag.longer, aes(x= sample, y= proportion))+
  ...
  labs(y="Non-primary to Primary Reads (%)")
