...
samples <- samples[!grepl("_rna_filtered", samples)]
...
sample <- "Q36_Copenhageni_Basecalled_May_31_2020_Direct-RNA"
for (sample in samples){
  ...
  temp <- s[1]
  label = paste(sample_abb, temp, sep="-")
  
  temp_genome_data <- readLines(file.path(polyapath, "genome", "sam_stats", paste0(sample,"_rna_filtered.sam.stats")))
  temp_trans_data <- readLines(file.path(polyapath,"transcriptome", "sam_stats", paste0(sample,".sam.stats")))
  ...
  total.reads <- rbind(total.reads,data.frame(sample=label, total.reads=c(temp_sn_genome[1,]$Value)))
  # -F 2308, all primary reads,excluding reads that are not primarily aligned and supplementary alignment (mapped)
  mapped.reads.genome <- rbind(mapped.reads.genome,data.frame(sample=label, mapped.reads.genome=c(temp_sn_genome[7,]$Value)))
  mapped.reads.trans <- rbind(mapped.reads.trans,data.frame(sample=label, mapped.reads.trans=c(temp_sn_trans[7,]$Value)))
  
  # samtools flagstats results for secondard and supplementary reads stats
  flagstats.genome <- readLines(file.path(polyapath, "genome", "flagstat", paste0(sample,"_rna_filtered.flagstat")))
  mapped.second.genome <- rbind(mapped.second.genome,data.frame(sample=label, secondary.alignment=unlist(strsplit(flagstats.genome[2], split=" ", fixed=TRUE))[1]))
  mapped.supp.genome <- rbind(mapped.supp.genome,data.frame(sample=label,supplementary.alignment=unlist(strsplit(flagstats.genome[3], split=" ", fixed=TRUE))[1]))
  
    flagstats.trans <- readLines(file.path(polyapath, "transcriptome", "flagstat", paste0(sample,".flagstat.sam")))
  mapped.second.trans <- rbind(mapped.second.trans,data.frame(sample=label, secondary.alignment.trans=unlist(strsplit(flagstats.trans[2], split=" ", fixed=TRUE))[1]))
  mapped.supp.trans <- rbind(mapped.supp.trans,data.frame(sample=label,supplementary.alignment.trans=unlist(strsplit(flagstats.trans[3], split=" ", fixed=TRUE))[1]))
  
   mapped.percentage <- rbind(mapped.percentage,data.frame(sample=label,mapped.percentage=parse_number(unlist(strsplit(flagstats.genome[5], split=" ", fixed=TRUE))[5])))
     mapped.percentage.trans <- rbind(mapped.percentage.trans,data.frame(sample=label,mapped.percentage.trans=parse_number(unlist(strsplit(flagstats.trans[5], split=" ", fixed=TRUE))[5])))
}
...
map.stats.count.per.longer$sample <- factor(map.stats.count.per.longer$sample, levels=c("LIC-dRNA-R1-Q29","LIC-dRNA-R2-Q29","LIC-dRNA-R1-Q36", "LIC-dRNA-R2-Q36"))
