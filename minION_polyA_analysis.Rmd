...
```{r}
ggplot(directRna.nano.combined, aes(x=position))+
...
grid.newpage()
plot <- grid.draw(cbind(ggplotGrob(p1), ggplotGrob(p2), size = "last"))
plot
...
}))  %>% subset(num_reads_mapped >5)
longest_polyA_median
top_10_median <- head(unique(longest_polyA_median$median_polyA_length), 10)
ggplot(longest_polyA_median %>% subset(median_polyA_length %in% top_10_median), aes(x=plot_label,y=polya_length ))+
  geom_violin()+
  geom_boxplot(width = 0.3) +
