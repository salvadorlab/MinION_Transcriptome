...
```
# Format Taxa file for dictionary
```{r}
library(dplyr)
library(tidyverse)
...
length(BC03.noDup.map$READID) 
length(unique(BC03.noDup.map$READID))
...
bc3.p
 ...
 as.tibble(BC03.lepto.tree.reroot)
...
# check number of reads assigned to at least family level
length(BC04.noDup.map$READID) 
length(unique(BC04.noDup.map$READID))
...
# check number of reads assigned to at least family level
length(BC05.noDup.map$READID) 
length(unique(BC05.noDup.map$READID))
...
# check number of reads assigned to at least family level
length(BC06.noDup.map$READID) 
length(unique(BC06.noDup.map$READID))
