...
rld <- rlog(salmon.deseq, blind=FALSE) # input is stringite output
...
cyt.go <- salmonGoDF[!is.na(salmonGoDF$GO_terms),]$GO_terms
head(cyt.go)
term <- select(GO.db, keys=cyt.go, columns="TERM")
term

egg_nog_go
table(Ontology(unique(unlist(strsplit(salmonGoDF$GO_terms,",")))))
