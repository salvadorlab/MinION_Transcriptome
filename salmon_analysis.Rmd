...
head(egg_nog_KEGG)
go_dict <- setNames(split(egg_nog_KEGG, seq(nrow(egg_nog_KEGG))), egg_nog_KEGG$gene_name)

kegg_gene2cat <- sapply(go_dict, function(item){
...
txsByGene <- transcriptsBy(txdb,c("gene", "exon", "cds"))
...
### use go seq to identify the most enriched pathway
```{r}
...
goseq_res_BP <- goseq_res %>% subset(ontology == "BP")
...
ggplot(upreg.count_longer,aes(x=temperature, y=count,shape=replicate))+
  ...
  theme_bw() +
  labs(x="Normalized Counts Per Million (Counts)")
...
pwf
length(go_gene2cat)
goseq_res <- goseq(pwf,gene2cat = go_gene2cat)
goseq_res_BP <- goseq_res %>% subset(ontology == "BP")
goseq_res_BP$padj <- p.adjust(goseq_res_BP$over_represented_pvalue,method="BH")
goseq_res$padj <- p.adjust(goseq_res$over_represented_pvalue,method="BH")

goseq_res_BP[order(goseq_res_BP$padj),] 
goseq_res[order(goseq_res$padj),]
