...
count.df.annot %>% subset(salmon_tax=="gene-LIC_RS11285")
```
...
pwf
length(go_gene2cat)
goseq_res <- goseq(pwf,gene2cat = ko_gene2cat, use_genes_without_cat=TRUE)
goseq_res_BP <- goseq_res %>% subset(!(numDEInCat <= 4))#%>% subset(ontology == "BP")
goseq_res_BP$padj <- p.adjust(goseq_res_BP$over_represented_pvalue,method="BH")
...
kegg.genes.enriched <- index$map04212

y$counts[c(kegg.genes.enriched),]
edgeR2ResDF[c(kegg.genes.enriched),]
```
