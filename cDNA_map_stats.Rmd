...
stat.test <- rbind( data.frame(Strain = unique(avg_primary_align$Strain), bins="(0,100]", `.y.`="coverage.fraction", group1="NoPolyATail", group2="PolyATail", n1 = 0, n2 = 0, statistic = 0, df=0, p=0, p.adj = 0,p.adj.signif="ns", y.position = 1.111, groups = c("NoPolyATail", "PolyATail"), x=1, xmin=0.8, xmax=1.2), stat.test)


p1 <- ggplot(avg_primary_align, aes(x=bins, y= coverage.fraction))+
  geom_boxplot(position = position_dodge(preserve = "single"),outlier.size = 1, aes(fill=is.polyA))+
  geom_text(data = avg_primary_summary, aes(x=bins, y=1, label=num_transcripts), position = position_dodge(width = .8), vjust =-0.4,size=3)+
  facet_wrap(~Strain) +
  theme_bw()+
  ...
  scale_y_continuous(labels = comma,breaks=seq(0, 1, by = 0.25),limits = c(0, 1.2))+
...
library(dplyr)
library(ggplot2)
primary.genome.map.stats <- read.csv(file.path(cdna.path, "genome/cDNA_rna_filtered_genome_map_statistics.csv"))%>% subset(!(is_secondary == "True" | is_supplementary == "True"))
...
samples <- sapply(list.files(file.path(cdnapath, "genome", "flagstat")), function(x){
  s <- unlist(strsplit(x, split=".", fixed=TRUE))[1]
  y <- unlist(strsplit(s, split="_", fixed=TRUE))
  paste(y[seq(1,length(y)-2)], collapse="_")
})
...
for (index in 1:length(samples)){
  ...
  temp_genome_data <- readLines(file.path(cdnapath, "genome","sam_stats", paste0(sample,"_rna_filtered",".sam.stats")))
  ...
  flagstats.genome <- readLines(file.path(cdnapath,"genome" ,"flagstat", paste0(sample,"_rna_filtered",".flagstat")))
...
# write.table(copen.no.avg, file.path(gen.path, "qualimap/Copenhageni_Basecalled_Aug_16_2019_Direct-cDNA_NoPolyATail_rna_filtered/raw_data_qualimapReport/nopolyAavg_copen_coverage_across_reference.txt"), sep="\t", row.names = FALSE, quote=FALSE)
...
plot_cov <- function(file,plotTitle ,genSize, chrIsize){  
  ...
  p1 <- ggplot(ref.cov, aes(x=X.Position..bp., y=Coverage))+
    ...
    scale_y_continuous(labels = comma,breaks=seq(0, 1500, by = 100),limits = c(0, 1700))+
    ...
    annotate("text", x=0, y=1000, label= "Chrom I",angle = 90, color="red") +
    annotate("text", x=genSize - 150000, y=1000, label= "Chrom II", angle = 90, color="red") +
...
# ggsave(file.path(man.path, "figures/I.3.full.genome.cov.to.ref.notlog.pdf"), combined_p, width = 15, height = 13)
...
rna_filtered_cdna <- read.csv(file.path(cdna.path, "genome/cDNA_rna_filtered_genome_map_statistics.csv"))
mean(rna_filtered_cdna$GC.content) #38.87811
sd(rna_filtered_cdna$GC.content)#4.482051
