...
for(file in list.dirs(file.path(qualimap),recursive = FALSE)){
  ...
  top.trans <- trans.stats.anot %>% 
    ...
    head(10) %>% 
    select(protein_id, protein_name, start, end, strand, `Mean coverage`) %>%
...
all.top.trans.edited <-  all.top.trans %>% group_by(sample, protein_id) %>% mutate(`Mean coverage` = mean(`Mean coverage`)) %>% distinct()
all.top.trans.edited <- all.top.trans.edited %>% mutate(label = paste(protein_id, protein_name, paste(start, end, sep="-"), sep="\n"))

all.top.trans.wider <-all.top.trans.edited %>% select(sample, protein_id, `Mean coverage`) %>% pivot_wider(names_from = sample, values_from = `Mean coverage`)
all.top.trans.wider[is.na(all.top.trans.wider$protein_id),]$protein_id <- "Hypothetical genes"
